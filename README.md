...
很多东西费时费力，要选择性的复制别人的代码，但是文件是自己创建的，项目也得自己搭建，一定要对每一个操作了如指掌
...
export default {
  ...
  data() {
    return {
      menus: [],//三级分类菜单数据
      defaultProps: {//默认规则
        children: 'children',//指定子树为节点对象的某个属性值（后端封装好实体中有children集合）
        label: 'name'//指定节点标签为节点对象的某个属性值（集合中每一个实体都显示name属性）
      },
      expandedKey: [],//设置展开的菜单数据
      dialogVisible: false,//默认对话框隐藏
      category: {
        name: "",
        parentCid: 0,
        catLevel: 0,
        ...
        icon: "",
        productUnit: ""
      },//新增对话框绑定的对象，v-model双向绑定name这个属性,后面四个默认属性
      title: "",//对话框的标题
      dialogType: "",//对话框的类型，append/edit
    };
  },
  //计算属性 类似于data概念
  computed: {},
  ...
  methods: {
    ...
    //这个方法的最终目的就是请求服务数据，响应数据给到vue实例menus，然后menus到了数据存放处，再通过template渲染
    //查询
    getMenus() {
      this.$http({
        url: this.$http.adornUrl("/product/category/list/tree"),//http://localhost:10000/renren-fast/product/category/list/tree
        method: "get"
      }).then(({ data }) => {// success 响应到数据后填充到绑定的标签中，上面获取到的对象有很多内容，包括data，data中还有data，这才是最终要显示的内容，所以把第一个data从对象中结构出来
        console.log("成功获取到菜单数据...", data.data);
        this.menus = data.data;//把数据给menus，就是给了vue实例，最后绑定到视图上
      });
    },
    ...
    //新增
    addCategory() {
      console.log("提交的三级分类数据", this.category)//打印提交内容的对象数据
      //发送post请求
      this.$http({
    ...
    //弹出新增对话框
    append(data) {
      console.log("append", data)//打印当前分类的数据
      this.dialogVisible = true;//调用该方法会打开对话框
      this.dialogType = "append";//设置对话框类型
      this.title = "新增";//设置对话框标题
      this.category.parentCid = data.catId;//新增分类的父id
      this.category.catLevel = data.catLevel * 1 + 1;//新增分类的层级
      this.category.name = "";
      this.category.catId = null;//自动递增的所以null
      this.category.icon = "";
    ...
    //提交对话框的数据
    submitData() {
      if (this.dialogType == "append") {//对话框类型是
        //就调用新增
        this.addCategory();
